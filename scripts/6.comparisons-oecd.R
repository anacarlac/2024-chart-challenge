...
graph_govspend <- data_govspend %>% 
  ...
  dplyr::filter(year != "2009") %>% 
  dplyr::filter(year != "2010") %>% 
  dplyr::filter(edu_level == "Primary to post-secondary non-tertiary education") %>% 
  ggplot(aes(y=gov_spending, x=year, color=coutries)) + 
  geom_line(size = 1, alpha = 0.8) +
  # geom_line(size = 3, alpha = 0.05) +
  # geom_line(size = 2, alpha = 0.1) +
  ...
  labs(
    title = "How much are we investing in education? \nResults for primary to post-secondary non-tertiary education",
    subtitle = "Total public expenditure on education as a percentage of total government expenditure in Brazil and OECD countries",
    caption = "Visualization by Ana Crispim. \nSource: OECD (2024), Education at a glance: Educational finance indicators, OECD Education Statistics (database),\nhttps://doi.org/10.1787/c4e1b551-en (accessed on 06 April 2024).",
    x = " ", y = " ",
    color = " "
  ) +
  theme(
...
# Save plot
ggsave("outputs/plot-day6-comparisons-oecd-primary-using-r.png", width = 6, height = 4)
